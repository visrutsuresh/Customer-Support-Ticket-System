...
def map_category(cat: str) -> str:
    ...
    c = cat.lower()
    if "refund" in c or "cancel" in c:                          return "refund"
    if "payment" in c or "invoice" in c or "subscription" in c: return "billing"
    if "shipping" in c or "delivery" in c:                      return "shipping"
    if "account" in c:                                          return "account"
    if "feedback" in c:                                         return "complaint"
    return "general"
...
def load_rows() -> list[dict]:
    ...
    rows = [{"instruction": r["instruction"], "response": r["response"],
             "category": r["category"], "intent": r["intent"]} for r in ds]
