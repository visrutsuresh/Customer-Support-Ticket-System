...
| R-3 | Quality claims cannot be defended because there is no labelled evaluation set | High | Medium | State plainly that accuracy is unmeasured, and report only rates and latencies. Building a small labelled set is the highest-value remaining work | Build owner |
